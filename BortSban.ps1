    ...
    PROCESS {
        ...
        try
        {
            ...
            if ($XmlOut) {
                ...
                $xmlStream.WriteStartElement("BortSbanrun")
            ...
            $startMsg = "Invk-BortSban.ps1 v$version scan initiated $startdate as: $myInvocationLine"

            #TODO deal with output
            Write-BortSbanOut -comment $startMsg -grepStream $grepStream -xmlStream $xmlStream -readableStream $readableStream
...
"@
                ...
                if(!$SkipDiscovery)
                {
                    [bool] $hostResult = BortSban-Alive $thost
                    $openPorts.clear()
                    $closedPorts.clear()
                    $filteredPorts.Clear()
                }
            ...
            while (($saveIteration * $SyncFreq) -lt $hostList.Count)
            {
                ...
                foreach ($iHost in $hostList[$sIndex..$eIndex])
                {
                    ...
                    Start-Job -ScriptBlock $BortSbanCode -Name $iHost -ArgumentList @($iHost, $SkipDiscovery, $PingOnly, $Timeout, $portList, $hostPortList, $Threads)  | Out-Null
                }
                ...
                foreach ($job in Get-Job)
                {
                    ...
                    Write-BortSbanOut -outhost $jobName -isUp $hostUp -openPorts $openPorts -closedPorts $closedPorts -filteredPorts $filteredPorts -grepStream $grepStream -xmlStream $xmlStream -readableStream $readableStream -SkipDiscovery $SkipDiscovery
                }
            ...
            Write-BortSbanOut -comment $endMsg -grepStream $grepStream -xmlStream $xmlStream -readableStream $readableStream
